...
printf "?name=WonLifeLinux-${gameClientVersion}.${patchVersion}.zip" >> upload-url.txt
cat upload-url.txt
curl --request POST `cat upload-url.txt` -K "/ohol/config/curl_github_auth.txt" -H "Content-Type: application/zip" --data-binary @"WonLifeLinux-${gameClientVersion}.${patchVersion}.zip"
